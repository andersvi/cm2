    ...
    (do ((stop #f))
      ...
      (case (car forms)
        ...
        (( ahead )
         ...
         (when (null? (cdr forms))
           (loop-error ops head "Missing '" oper "' expression."))
         (set! args (append args (list ':ahead (cadr forms))))
         (set! forms (cddr forms)))
        (else
         (set! stop #t))))
    (case oper
...
(define *each-operators*
  (list (list 'as (function parse-for)
        ...
        (list 'output (function parse-process-clause) 'task 'to 'at 'ahead)
        ;; sprout's ahead not support yet.
