...
(defparameter toplevel-translations
  '((any                 some)
    ...
    (bound?              boundp)
    (call-with-output-file nil call-with-output-file->with-open-file)
...
;;;
;;; optional stocl-package declaration to be written at top of output file.
;;;

(defparameter stocl-package ':cm)
...
(defparameter stocl-header
  ";;; **********************************************************************
;;; Copyright (C) ~D Heinrich Taube, <taube (at) uiuc (dot) edu>
;;;
;;; This program is free software; you can redistribute it and/or
    ...
    (flet ((tracename (form)
      ...
      (with-open-file (in scm)
        (with-open-file (out lisp :direction :output
                             ...
                             :if-exists :supersede)
          (format out stocl-header (datestr t))
          (format out
		  "~%;;; generated by scheme->cltl from ~A.~A on ~A~%~%"
...
(defun call-with-output-file->with-open-file (form &optional env)
  ;;(call-with-output-file filename (lambda (file) ...))
  ;; => (with-open-output (file filename :direction :output) ...)
  (let* ((file (scheme->cltl (second form) env))
	 (func (third form))
	 (port (first (second func))))
